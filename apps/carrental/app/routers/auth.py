...
def verify_otp(payload: VerifyOtpIn, request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        role = payload.role if payload.role in ("renter", "seller") else "renter"
...
def dev_login(payload: DevLoginIn, db: Session = Depends(get_db)):
    ...
    users = {
        ...
        # Service-specific
        "renter": {"password": "renter", "phone": "+963900700101", "name": "Dev Renter", "role": "renter"},
        "seller": {"password": "seller", "phone": "+963900700201", "name": "Dev Seller", "role": "seller"},
    }
