...
def receive_webhook(
    ...
    if payload.get("type") in ("requests.accept", "requests.accepted"):
        ...
        if pr_id:
            # Match reservation first
            res = db.query(Reservation).filter(Reservation.payment_request_id == pr_id).one_or_none()
            if res is not None:
                if transfer_id:
                    res.payment_transfer_id = transfer_id
                db.flush()
            else:
                ent = db.query(Entry).filter(Entry.payment_request_id == pr_id).one_or_none()
                if ent is not None:
                    if transfer_id:
                        ent.payment_transfer_id = transfer_id
                    db.flush()
    return {"detail": "ok"}
