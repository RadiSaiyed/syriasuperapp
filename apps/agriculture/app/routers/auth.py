...
from ..middleware_rate_limit import send_and_store_otp, verify_otp_code, consume_otp
...
def verify_otp(payload: VerifyOtpIn, request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        role = payload.role if payload.role in ("buyer", "farmer", "worker") else "buyer"
...
def dev_login(payload: DevLoginIn, db: Session = Depends(get_db)):
    ...
    users = {
        ...
        # Service-specific
        "buyer": {"password": "buyer", "phone": "+963900800101", "name": "Dev Buyer", "role": "buyer"},
        "farmer": {"password": "farmer", "phone": "+963900800201", "name": "Dev Farmer", "role": "farmer"},
        "worker": {"password": "worker", "phone": "+963900800301", "name": "Dev Worker", "role": "worker"},
    }
