...
class Settings:
    ...
    APP_PORT: int = int(os.getenv("APP_PORT", "8096"))
    DB_URL: str = os.getenv(
        "DB_URL",
        "postgresql+psycopg2://postgres:postgres@localhost:5436/parking_onstreet",
    )
    ...
    RATE_LIMIT_REDIS_PREFIX: str = os.getenv("RATE_LIMIT_REDIS_PREFIX", "rl_parking_onstreet")
    ...
    OPERATOR_PHONE: str = os.getenv("OPERATOR_PHONE", "+963900000001")
    FEE_WALLET_PHONE: str = os.getenv("FEE_WALLET_PHONE", "+963999999999")
    # Internal API auth for orchestrators (e.g., AI Gateway)
    INTERNAL_API_SECRET: str = os.getenv("INTERNAL_API_SECRET", "dev_ai_secret")
    REDIS_URL: str = os.getenv("REDIS_URL", "redis://redis:6379/0")
    AI_GATEWAY_BASE_URL: str = os.getenv("AI_GATEWAY_BASE_URL", "http://localhost:8099")
...
if not settings.DEV_MODE:
    ...
    if not os.getenv("JWT_JWKS_URL") and (
        not settings.JWT_SECRET or settings.JWT_SECRET == "change_me_in_dev"
    ):
        raise RuntimeError("Configure JWT_SECRET or JWT_JWKS_URL when ENV!=dev")
