    ...
    try:
        ...
        if jwks_url:
            ...
            aud = (
                ...
                if os.getenv("JWT_VALIDATE_AUD", "false").lower() == "true"
                else None
            )
            iss = (
                os.getenv("JWT_ISSUER")
                if os.getenv("JWT_VALIDATE_ISS", "false").lower() == "true"
                else None
            )
...
def get_current_user(
    ...
    if not user:
        # Auto‑provision minimal user for Super‑App propagated tokens
