...
def verify_otp(payload: VerifyOtpIn, request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        user = User(phone=payload.phone, name=payload.name or None, role="shipper")
        db.add(user)
        db.flush()
...
def dev_login(payload: DevLoginIn, db: Session = Depends(get_db)):
    ...
    users = {
        ...
        # Service-specific
        "shipper": {"password": "shipper", "phone": "+963900300101", "name": "Dev Shipper", "role": "shipper"},
        "carrier": {"password": "carrier", "phone": "+963900300201", "name": "Dev Carrier", "role": "carrier"},
    }
