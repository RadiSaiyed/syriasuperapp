...
def register_token(payload: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if row is None:
        row = DeviceToken(user_id=user.id, token=token, platform=platform, app_mode=app_mode, enabled=True, last_seen=datetime.now(timezone.utc), updated_at=datetime.now(timezone.utc))
        db.add(row)
    else:
        row.user_id = user.id
