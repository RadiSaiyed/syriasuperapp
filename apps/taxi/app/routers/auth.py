...
from ..otp_utils import send_and_store_otp, verify_otp_code, consume_otp
...
def verify_otp(payload: VerifyOtpIn, request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        user = User(phone=payload.phone, name=payload.name or None, role="rider")
        db.add(user)
        db.flush()
...
def dev_login(payload: DevLoginIn, db: Session = Depends(get_db)):
    """Development-only username/password login mapping to fixed test users.
    Creates the user on first login and returns a JWT. This avoids OTP/phone
