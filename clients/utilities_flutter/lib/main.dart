...
class _AppState extends State<App> {
  ...
  late ApiClient _api;
  String _baseUrl = 'http://localhost:8084';
  static const _baseUrlKey = 'utilities_base_url';
  int _tab = 0;
    ...
    return MaterialApp(
      ...
      home: FutureBuilder<String?>(
        ...
        builder: (context, snap) {
          ...
          return Scaffold(
            appBar: AppBar(title: const Text('Utilities MVP'), actions: [
              IconButton(
                ...
                onPressed: () async {
                  ...
                  final url = await showDialog<String>(
                    ...
                    builder: (_) => AlertDialog(
                      ...
                      content: TextField(controller: ctrl, decoration: const InputDecoration(hintText: 'http://localhost:8084')),
                      actions: [
            ...
            body: loggedIn
                ? Column(children: [
                    NavigationBar(
                      ...
                      destinations: const [
                        NavigationDestination(icon: Icon(Icons.apartment), label: 'Billers'),
                        NavigationDestination(icon: Icon(Icons.receipt_long), label: 'Bills'),
                        NavigationDestination(icon: Icon(Icons.phone_android), label: 'Top-up'),
                        NavigationDestination(icon: Icon(Icons.history), label: 'Top-ups'),
                      ],
                    ),
                    const Divider(height: 1),
                    Expanded(child: _tab == 0 ? BillersScreen(api: _api) : _tab == 1 ? BillsScreen(api: _api) : _tab == 2 ? TopupScreen(api: _api) : TopupsScreen(api: _api)),
                  ])
