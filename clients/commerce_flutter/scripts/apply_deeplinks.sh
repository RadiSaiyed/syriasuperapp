...
echo "Applying deep-link launch whitelist for Commerce app (iOS)..."

# iOS: whitelist payments scheme for canLaunchUrl (use Python plistlib)
if [[ -f "$IOS_PLIST" ]]; then
...
PY
  ...
  echo "! Info.plist not found. Run: flutter create . inside commerce_flutter first."
fi

echo "Done."

echo "Applying Android queries visibility for Commerce app (Android)..."

# Android: declare visibility to query payments scheme for canLaunchUrl
  ...
  if ! grep -q '<queries>' "$ANDROID_MANIFEST"; then
    ...
    if ! grep -q 'android:scheme="payments"' "$ANDROID_MANIFEST"; then
      ...
      awk '
        {print}
        $0 ~ /<queries>/ {flag=1}
        flag==1 && $0 ~ /<\/queries>/ && done==0 {
          print "  <intent>\n    <action android:name=\"android.intent.action.VIEW\" />\n    <data android:scheme=\"payments\" />\n  </intent>"; done=1
        }
      ' "$ANDROID_MANIFEST" > "$ANDROID_MANIFEST.tmp" && mv "$ANDROID_MANIFEST.tmp" "$ANDROID_MANIFEST"
    else
      echo "- AndroidManifest already declares payments scheme in <queries>. Skipping."
    fi
  fi
else
  echo "! AndroidManifest.xml not found. Run: flutter create . inside commerce_flutter first."
fi
