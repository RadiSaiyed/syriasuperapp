...
class _BookingsScreenState extends State<BookingsScreen> {
  ...
  Future<void> _ticket(String id) async {
    ...
    try {
      ...
      setState(() => _ticketText = (t['qr_text'] as String?));
      if (!mounted) return;
      await showModalBottomSheet(context: context, showDragHandle: true, builder: (ctx) => Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Ticket QR Text', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        SelectableText(_ticketText ?? ''),
        const SizedBox(height: 8),
        Row(children: [
          TextButton.icon(onPressed: () async { if (_ticketText != null) { await Clipboard.setData(ClipboardData(text: _ticketText!)); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Copied ticket text')));} }, icon: const Icon(Icons.copy), label: const Text('Copy')),
        ])
      ])));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e')));
        ...
        child: RefreshIndicator(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              final b = _items[i];
              return ListTile(
                title: Text('${b['origin']} → ${b['destination']} • ${b['operator_name']}'),
