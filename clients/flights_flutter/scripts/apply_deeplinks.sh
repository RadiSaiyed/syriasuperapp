...
echo "Applying deep-link launch whitelist for Flights app (iOS)..."

# iOS: whitelist payments scheme for canLaunchUrl (safe via plistlib)
if [[ -f "$IOS_PLIST" ]]; then
...
PY
  ...
  echo "! Info.plist not found. Run: flutter create . inside flights_flutter first."
fi

echo "Applying Android queries visibility for Flights app (Android)..."
...
if [[ -f "$ANDROID_MANIFEST" ]]; then
  ...
  echo "! AndroidManifest.xml not found. Run: flutter create . inside flights_flutter first."
fi
