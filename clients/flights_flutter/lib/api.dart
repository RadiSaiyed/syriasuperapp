...
class ApiClient {
  ...
  Future<List<Map<String, dynamic>>> searchFlights({required String origin, required String destination, required DateTime date}) async {
    ...
    final res = await http.post(Uri.parse('$baseUrl/flights/search'), headers: await _authHeaders(), body: jsonEncode({'origin': origin, 'destination': destination, 'date': dateStr}));
    if (res.statusCode >= 400) throw ApiError('Search failed: ${res.body}');
    final data = jsonDecode(res.body) as Map<String, dynamic>;
    final items = (data['flights'] as List? ?? []).cast<dynamic>().map((e) => (e as Map).cast<String, dynamic>()).toList();
    return items;
  }
  ...
  Future<Map<String, dynamic>> createBooking({required String flightId, int seatsCount = 1, List<int>? seatNumbers, String? promoCode}) async {
    final body = {'flight_id': flightId, 'seats_count': seatsCount};
  ...
  Future<Map<String, dynamic>> flightSeats(String flightId) async {
    final res = await http.get(Uri.parse('$baseUrl/flights/$flightId/seats'), headers: await _authHeaders());
    if (res.statusCode >= 400) throw ApiError('Flight seats failed: ${res.body}');
    return (jsonDecode(res.body) as Map).cast<String, dynamic>();
  }
