...
class Glass extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveOpacity = isDark ? (opacity < 0.30 ? 0.30 : opacity) : (opacity < 0.24 ? 0.24 : opacity);
    return Container(
      margin: margin,
          ...
          child: Stack(children: [
            DecoratedBox(
              decoration: BoxDecoration(
                ...
                borderRadius: borderRadius,
                border: Border.all(color: Colors.white.withValues(alpha: isDark ? 0.24 : 0.32), width: 1.1),
                boxShadow: [
                  BoxShadow(color: Colors.black.withValues(alpha: isDark ? 0.55 : 0.14), blurRadius: 44, offset: const Offset(0, 12)),
                ],
                gradient: LinearGradient(
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight,
                  colors: [
                    Colors.white.withValues(alpha: isDark ? (effectiveOpacity * 1.00) : (effectiveOpacity + 0.08)),
                    Colors.white.withValues(alpha: isDark ? (effectiveOpacity * 0.85) : (effectiveOpacity + 0.04)),
                  ],
                ),
              ),
            ),
            const FrostNoise(opacity: 0.045, scale: 6),
            Positioned.fill(
              child: DecoratedBox(
                decoration: BoxDecoration(
                  borderRadius: borderRadius,
...
class GlassCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Glass(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: child);
}
...
class _LiquidBackgroundState extends State<LiquidBackground> with SingleTickerProviderStateMixin {
  late final AnimationController _ctrl = AnimationController(vsync: this, duration: const Duration(seconds: 16))..repeat(reverse: true);
  late final Animation<double> _t = CurvedAnimation(parent: _ctrl, curve: Curves.easeInOut);
  @override
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final c1 = const Color(0xFF0A84FF); // blue
    final c2 = const Color(0xFF30D158); // green
    final c3 = const Color(0xFFFF9F0A); // orange
    final c4 = const Color(0xFF64D2FF); // light‑cyan
      ...
      builder: (_, __) {
        ...
        return Stack(children: [
          ...
          Positioned(
            ...
            child: _blob(size: 220, colors: [c1.withValues(alpha: isDark ? 0.22 : 0.06), c3.withValues(alpha: isDark ? 0.18 : 0.05)]),
          ),
          Positioned(
            bottom: -60 + math.cos(p * math.pi * 2) * 10,
            right: -20 + math.sin(p * math.pi * 2) * 8,
            child: _blob(size: 260, colors: [c2.withValues(alpha: isDark ? 0.20 : 0.05), c4.withValues(alpha: isDark ? 0.20 : 0.05)]),
          ),
        ]);
      },
    );
...
class LiquidGlassOverlay extends StatelessWidget {
  final double blur;
  final double opacity;
  const LiquidGlassOverlay({super.key, this.blur = 48, this.opacity = 0.24});
  ...
  Widget build(BuildContext context) {
    return IgnorePointer(
      ...
      child: Stack(children: [
        ...
        const FrostNoise(opacity: 0.03, scale: 5),
        Positioned.fill(
          child: DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
...
class FrostNoise extends StatelessWidget {
  ...
  final double scale;
  final int seed;
  const FrostNoise({super.key, this.opacity = 0.04, this.scale = 6, this.seed = 1337});
...
class _FrostNoisePainter extends CustomPainter {
  ...
  final double scale;
  final int seed;
  _FrostNoisePainter({required this.opacity, required this.scale, required this.seed});
  ...
  bool shouldRepaint(covariant _FrostNoisePainter oldDelegate) {
    return oldDelegate.opacity != opacity || oldDelegate.scale != scale || oldDelegate.seed != seed;
  }
}
